...
function applyFilters(skills: Skill[], query: string, filters: Filters): Skill[] {
  const q = query.toLowerCase().trim();
  return skills.filter((s) => {
    if (q) {
      const searchable = [
        s.title, s.description, ...s.tags,
        ...s.steps.map((st) => st.title + ' ' + st.description + ' ' + st.prompt_text),
      ].join(' ').toLowerCase();
      if (!searchable.includes(q)) return false;
    }
    if (filters.subjects.length > 0 && !filters.subjects.includes(s.subject)) return false;
    if (filters.levels.length > 0 && !filters.levels.includes(s.level)) return false;
    if (filters.tools.length > 0 && !filters.tools.includes(s.tool)) return false;
    if (filters.useCases.length > 0 && !filters.useCases.includes(s.use_case)) return false;
    return true;
  });
}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="max-w-2xl">
            ...
            <h1 className="font-serif text-5xl sm:text-6xl font-bold text-foreground mb-5 leading-none">
              Monivaiheisia
              <br />
              <span className="keyword-highlight text-teal">oppimisen reseptejä</span>
            </h1>
            <p className="text-lg text-muted mb-10 leading-relaxed max-w-lg">
              Skillit ovat vaiheistettuja työnkulkuja, joissa kukin vaihe sisältää oman promptinsa.
              Noudata järjestyksessä tai hypi suoraan tarvitsemaasi vaiheeseen.
            </p>
          ...
          <div className="flex-1 min-w-0" ref={resultsRef}>
            <div className="flex items-center justify-between mb-7 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  Suodata
                  {activeFilterCount > 0 && (
                    <span className="inline-flex items-center justify-center w-5 h-5 text-xs font-bold bg-teal text-white rounded-full">{activeFilterCount}</span>
                  )}
                </button>
              </div>
            </div>
            ...
            {results.length > 0 ? (
              ...
              <div className="text-center py-24">
                ...
                <p className="text-muted text-sm mb-7">Kokeile eri hakusanaa tai poista suodattimia.</p>
                <button
                  onClick={() => { handleQueryChange(''); handleFilterChange(EMPTY_FILTERS); }}
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 bottom-0 w-80 bg-background border-l border-warm-border overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setSidebarOpen(false)} className="text-muted hover:text-foreground transition-colors" aria-label="Sulje">
