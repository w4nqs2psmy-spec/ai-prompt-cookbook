...
function applyFilters(codes: Code[], query: string, filters: Filters): Code[] {
  const q = query.toLowerCase().trim();
  return codes.filter((c) => {
    if (q) {
      const searchable = [c.title, c.description, c.code_text, ...c.tags]
        .join(' ')
        .toLowerCase();
      if (!searchable.includes(q)) return false;
    }
    if (filters.subjects.length > 0 && !filters.subjects.includes(c.subject)) return false;
    if (filters.levels.length > 0 && !filters.levels.includes(c.level)) return false;
    if (filters.tools.length > 0 && !filters.tools.includes(c.tool)) return false;
    if (filters.useCases.length > 0 && !filters.useCases.includes(c.use_case)) return false;
    return true;
  });
}
...
export default function CodeHeroSearch({
  initialCodes,
  profileMap = {},
  initialQuery = '',
  initialPage = 1,
  ...
  const results = useMemo(() => {
    const filtered = applyFilters(initialCodes, query, filters);
    if (sortBy === 'popular') {
    ...
    return filtered;
  }, [initialCodes, query, filters, sortBy]);
  ...
  const baseLabel =
    totalCount === 1
      ? '1 tulos'
      : `${totalCount} ${query || activeFilterCount > 0 ? 'tulosta' : 'koodia'}`;
        ...
        <div className="max-w-7xl mx-auto">
          <div className="max-w-2xl">
            ...
            <p className="text-base text-muted mb-7 leading-relaxed max-w-lg">
              Valmiit koodiesimmerkit ja snippetit. Jokainen koodi on testattu ja
              optimoitu suomalaiseen opetuskontekstiin.
            </p>
                ...
                >
                  {pageResults.map((code, index) => (
                    ...
                    >
                      <CodeCard
                        code={code}
                        profile={code.submitted_by ? profileMap[code.submitted_by] : null}
                      />
                    </div>
                  ))}
                </div>
