...
import { Code, CODE_LANGUAGE_LABELS, SUBJECT_LABELS, LEVEL_LABELS, TOOL_LABELS, USE_CASE_LABELS } from '@/lib/types';
...
export default function CodeDetail({ code }: Props) {
  const [copied, setCopied] = useState(false);

  // Track view once per session
  useEffect(() => {
    trackView('code', code.id);
  }, [code.id]);

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(code.code_text);
    } catch {
      // fallback for older browsers
      const textarea = document.createElement('textarea');
      textarea.value = code.code_text;
      document.body.appendChild(textarea);
      textarea.select();
      document.execCommand('copy');
      document.body.removeChild(textarea);
    }
    trackCopy('code', code.id);
    setCopied(true);
